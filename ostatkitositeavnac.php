...
$aptekavita=1;							//брэнд аптека вита
$pervayasoc=2;							//брэнд первая социальная аптека


$procentvigruzki[0][0]=1000;						//минимальный проент наценки на товар БЕЗ БРЕНДА при котором партия выгружается на сайт для жизненно важных товаров
$procentvigruzki[0][1]=1000;						//минимальный проент наценки на товар БЕЗ БРЕНДА при котором партия выгружается на сайт для НЕ жизненно важных товаров
$procentvigruzki[$aptekavita][1]=6.5;				//минимальный проент наценки на товар при котором партия выгружается на сайт АПТЕКА ВИТА для жизненно важных товаров
$procentvigruzki[$aptekavita][0]=14;				//минимальный проент наценки на товар при котором партия выгружается на сайт АПТЕКА ВИТА для НЕ жизненно важных товаров
$procentvigruzki[$pervayasoc][1]=3;					//минимальный проент наценки на товар при котором партия выгружается на сайт ПЕРВОЙ СОЦИАЛЬНОЙ для жизненно важных товаров
$procentvigruzki[$pervayasoc][0]=7;					//минимальный проент наценки на товар при котором партия выгружается на сайт ПЕРВОЙ СОЦИАЛЬНОЙ для НЕ жизненно важных товаров
...
if($num_rows>0)
{
	...
	//	$str_to_log=showtime(0,time())."Ищем: ".$row["articul"]." с ноде: ".$row["node"].".\r\n";fwrite($fd, $str_to_log);
		...
		$ignorenacenka=0;																				//выгрузка в зависимости от величины наценки активна
	//	echo "ignorenacenka=$ignorenacenka<br>";
		...
		//echo "priceblock ignorenacenka=$ignorenacenka<br>";
		...
		//echo "comm ignorenacenka=$ignorenacenka<br>";
		...
		if($distprice>0)
		{
			...
			if($nacenka<>0)
			{
				$predelnacenki=$nacenka-$procentvigruzki[$brand][$isjvnl];
			}
		}
		if(($ignorenacenka==0)&&($predelnacenki<0))
		{
			echo "Предел наценки меньше нуля. партия не должна включаться в выгрузку на сайт!<br>";
			$loadtosite=0;
		}
		echo "nacenka=$nacenka<br>";
		echo "predelnacenki=$predelnacenki<br>";
		//echo $row["articul"]." ||| ".$row["type"]." ||| ".$predelnacenki."<br>";
		if($ignorenacenka>0)$predelnacenki=1.00;
		echo "predelnacenki=$predelnacenki<br>";
