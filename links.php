...
checkAuth();
    ...
    $querylink = "SELECT l.*, lt.name AS linktype_name 
              FROM `" . LINKSTABLE . "` l 
              LEFT JOIN `linktypes` lt ON l.linktype_id = lt.id";
    // Выполнение запроса
    ...
    if (isset($_GET['submit'])) {
        ...
        if (isset($_GET['modify'])){
            ...
            $link_for_delete = implode(",", $_GET['modify']);
            // Подключение к базе данных
            $connect = mysqli_connect(HOST, USER, PW, DB);
            // Установка кодировки соединения
            mysqli_set_charset($connect, "utf8");
            ...
            if ($_GET['submit'] == 'Удалить') {
                ...
                $select_positions = "SELECT `id` FROM `" . LINKSTABLE ."` WHERE `id` IN (" . $link_for_delete . ");";
                ...
                $query = "DELETE FROM `" . LINKSTABLE ."` WHERE `id` in ($for_delete);";
...
<form>
    ...
    <h3 style="text-align: center">Управление Ссылками</h3>
    <form id="mod" method="GET" action="links.php">
        <input type="submit" style="margin-left: 10px; margin-bottom: 10px;" class="btn btn-success add-btn" name="submit" value="Добавить ссылку" formaction="formlink.php">
    ...
    <table id="links" class="table table-bordered">
        <thead>
            <tr>
                ...
                <th style="padding: auto 0; width: 10%;">Тип ссылки</th>
                <th style="padding: auto 0; width: 8%;">ID Ссылки</th>
                <th style="padding: auto 0; width: 8%;">Ссылка</th>
                <th style="padding: auto 0; width: 8%;">Название Ссылки</th>
                <th style="padding: auto 0;">Описание Ссылки</th>
            </tr>
        </thead>
        <tbody>
            <?php
            foreach ($links as $one) {
                ...
                echo '<td><a href="formlink.php?id='.$one["id"].'" class="btn btn-warning btn-xs">Редактировать</td>';
                echo "<td>" . $one['linktype_name'] . "</td>";
                echo '<td>' . $one['id'] . '</td>';
                echo "<td><a target='_blank' href='" . $one['url'] . "'>" . $one['url'] . "</a></td>";
                echo "<td>" . $one['name'] . "</td>";
