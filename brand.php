<?php 
    ...
    // SQL запрос на выборку всех записей из таблицы brand
    $querycity = "SELECT * FROM `brand` WHERE 1";
    // Выполнение запроса
    ...
    if (isset($_GET['submit'])) {
        ...
        if (isset($_GET['modify'])){
            ...
            $brand_for_delete = implode(",", $_GET['modify']);
            // Подключение к базе данных
            $connect = mysqli_connect(HOST, USER, PW, DB);
            // Установка кодировки соединения
            mysqli_set_charset($connect, "utf8");
            ...
            if ($_GET['submit'] == 'Удалить') {
                ...
                $select_positions = "SELECT `id` FROM `" . BRANDTABLE ."` WHERE `id` IN (" . $brand_for_delete . ");";
                ...
                $query = "DELETE FROM `" . BRANDTABLE . "` WHERE `id` in ($for_delete);";
...
<head>
    ...
    <title>Управление Брендами</title>
...
?>
<form>
    <h3 style="text-align: center">Управление Брендами</h3>
    <form id="mod" method="GET" action="brand.php">
        <input type="submit" style="margin-left: 10px; margin-bottom: 10px;" class="btn btn-success add-btn" name="submit" value="Добавить бренд" formaction="formbrand.php">
    ...
    <table id="brand" class="table table-bordered">
        <thead>
            <tr>
                ...
                <th style="padding: auto 0; width: 8%;">ID Бренда</th>
                <th style="padding: auto 0;">Название Бренда</th>
            </tr>
        </thead>
        <tbody>
            <?php
            foreach ($brand as $one) {
                ...
                echo '<td><a href="formbrand.php?id='.$one["id"].'" class="btn btn-warning btn-xs">Редактировать</td>';
                echo '<td>' . $one['id'] . '</td>';
                echo "<td>" . $one['name'] . "</td>";
                echo "</tr>";
            }
