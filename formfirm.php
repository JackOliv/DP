...
if (isset($_GET['id'])) {
    $q = "SELECT `name` FROM " . FIRMTABLE . " WHERE `id` = {$_GET['id']}";
...
if(isset($_POST['submit'])) {
    ...
    if (isset($_GET['id'])){
        $query = "UPDATE `" . FIRMTABLE . "` SET 
            `name`='{$_POST['name']}'
            WHERE `id` = {$_GET['id']}";
    } else {
        // Если параметр id не передан, формируем запрос на добавление новой записи
        $query = "INSERT INTO `" . FIRMTABLE . "` (`name`) VALUES ('" . $_POST['name'] . "')";
        echo "<br>";
        echo $query;
        echo "<br>";
    }
    ...
    if (!$stmt->execute()) {
        ...
        // Перенаправление на страницу со списком фирм
...
<head>
    ...
    <title>Добавление Юр. лица</title>
